...
class Order(models.Model):
    ...
    currency = models.CharField(default="sar", max_length=8)  # lower case ISO currency codes
...
class CertificateItem(OrderItem):
    ...
    @classmethod
    @transaction.atomic
    def add_to_order(cls, order, course_id, cost, mode, currency='sar'):
        """
    ...
    def verified_certificates_contributing_more_than_minimum(cls, course_id):
        return use_read_replica_if_available(
            CertificateItem.objects.filter(
                ...
                mode='verified',
                status='purchased',
                unit_cost__gt=(CourseMode.min_course_price_for_verified_for_currency(course_id, 'sar')))).count()
...
class Donation(OrderItem):
    ...
    @classmethod
    @transaction.atomic
    def add_to_order(cls, order, donation_amount, course_id=None, currency='sar'):
